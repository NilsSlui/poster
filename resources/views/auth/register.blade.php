        ...
        <div class="w-8/12 bg-gray-400 p-6 rounded-lg">
            <form action="{{ route('register') }}" method="post">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="name" placeholder="Your name" class="bg-gray-100 border-2 w-full p-4 rounded-lg @error('username')border-red-500 @enderror" value="{{ old('name') }}">
                    @error('name')
                        <div class="text-red-500 mt-2 text-sm">
                            {{ $message }}
                        </div>
                ...
                <div>
                    <button type="submit" class="bg-indigo-500 text-white px-3 py-3 rounded font-medium w-full">Register</button>
            </form>
        </div>
    </div>
@endsection